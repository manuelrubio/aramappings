...
In the following example, we use 2 workers/cores when testing the code in CRAN. Otherwise we detect the available number of cores (`NCORES`) with package **parallelly** (if installed) or **parallel**, and use half of them (if several are available).
