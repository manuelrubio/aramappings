...
test_that("Function halts if V is not a matrix", {
  expect_error(ara_exact_l2(X, Vlist))
})

test_that("Function halts if variable is not numeric", {
  expect_error(ara_exact_l2(X, V, variable = "1"))
})

test_that("Function halts if solver is not a string", {
  expect_error(ara_exact_l2(X, V, solver = 1))
})
...
test_that("Function halts if selected variable is not in [1,n]", {
  expect_error(ara_exact_l2(X, V, variable = variable))
})
...
test_that("Function halts if the specified solver is not 'formula' or 'CVXR'", {
  expect_error(ara_exact_l2(X, V, solver = "some invalid solver"))
})
...
for (m in 1:3) {
  ...
  R_formula <- ara_exact_l2(X, V)
  R_CVXR <- ara_exact_l2(X, V, solver = "CVXR")
