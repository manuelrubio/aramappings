...
#'
#' @description
#' \code{ara_exact_Linf()} computes \strong{exact} \strong{Adaptable Radial Axes} (ARA) mappings for
#' the \strong{L-infinity norm}
#'
#' @details
#' \code{ara_exact_Linf()} computes low-dimensional point representations of high-dimensional
...
#' function solves the constrained optimization problem in Eq. (13), for the L-infinity vector norm. Its equality constraint
...
ara_exact_Linf_CVXR <- function(
  ...
  constraints <- list()
  constraints <- append(constraints, -tvar %*% pracma::ones(1, n) <= Pvar %*% t(V) - X)
  constraints <- append(constraints, Pvar %*% t(V) - X <= tvar %*% pracma::ones(1, n))
...
ara_exact_Linf_glpkAPI <- function(
  ...
  vals <- c(V[variable, ], coo_lists$vals)
  ...
  type_rows <- c(glpkAPI::GLP_FX, rep(glpkAPI::GLP_UP, 2 * n))
...
ara_exact_Linf_clarabel <- function(
  ...
  A <- rbind(cbind(0, t(V[variable, ])), cbind(matrix(-1, n, 1), -V), cbind(matrix(-1, n, 1), V))
...
ara_exact_Linf_Rglpk <- function(
  ...
  bounds <- list(lower = list(ind = 2:(1 + m), val = rep(-Inf, m)), upper = list())
