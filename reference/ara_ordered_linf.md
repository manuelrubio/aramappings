...
`ara_ordered_linf()` computes **ordered** **Adaptable Radial Axes**
(ARA) mappings for the **Linf norm**
...
`ara_ordered_linf()` computes low-dimensional point representations of
...
(14), for the L-infinity norm. The inequality constraint ensures that
the estimates for a selected variable are ordered in accordance with its
