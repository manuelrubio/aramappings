...
`ara_ordered_l2()` computes **ordered** **Adaptable Radial Axes** (ARA)
...
If the chosen solver fails to map the data (i.e., fails to solve the
related optimization problem), `P` will contain `NA` (not available)
values. In that case, `objval` will also be `NA`.

## Details

`ara_ordered_l2()` computes low-dimensional point representations of
...
(14), for the squared-Euclidean norm. The inequality constraint ensures
that the estimates for a selected variable are ordered in accordance
with its original values. In other words, ignoring any ties, the
estimate for the data observation with the i-th smallest value will
